...
For example, the [Brand Customizations Tutorial](https://docs.medusajs.com/learn/customization/extend-features) shows how to create a Brand Module that adds the concept of brands to your application, then link those brands to a product.
...
For example, if you define a link between the `Product` data model from the [Product Module](https://docs.medusajs.com/resources/commerce-modules/product) and a `Post` data model from a Blog Module, the table name would be `product_product_blog_post`.
...
You can also add custom columns in the link table as explained in the [Add Columns to Link Table chapter](https://docs.medusajs.com/learn/fundamentals/module-links/custom-columns).
