...
1. Data models collected in the [Query's graph](https://docs.medusajs.com/learn/fundamentals/module-links/query#querying-the-graph). These types provide you with auto-completion and type checking when using Query.
   - Generated data model types are located in the `.medusa/types/query-entry-points.d.ts` file.
2. Modules registered in the [Medusa container](https://docs.medusajs.com/learn/fundamentals/medusa-container). These types provide you with auto-completion and type checking when resolving modules from the container.
...
Instead, if you need to use a data model's type in your customizations, you can use the [InferTypeOf](https://docs.medusajs.com/learn/fundamentals/data-models/infer-type) utility function, which infers the type of a data model based on its properties.
